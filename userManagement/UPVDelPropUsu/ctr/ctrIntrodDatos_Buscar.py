...
class ctrIntrodDatos_Buscar(ctrIntrodDatos):
    ...
    def busca(self):
        ...
        if len(self.dicEnviar)==0:#no se ha rellenado ningun campo de busqueda
            ...
            self.ui.lbEstado.setText("Buscando coincidencias en la base de datos. Por favor espere ...")
            listaDic=self.oUtiles.oConsultasPg.recuperaDatosTablaByteaDic(self.nomTabla, self.listaNomCampos, condicionWhere,self.dicEnviar.values(),bytea_output_to_escape=False,orderBy=campoOrderBy,limit=limiteRegistros)
